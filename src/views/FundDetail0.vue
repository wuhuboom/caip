    ...
    >
      <div class="table-lists">
        <div class="row hearder">
          ...
          <div class="col">红包数量</div>
          <div class="col">已领取金额</div>
          <div class="col">派发时间</div>
          <div class="col">状态</div>
        </div>
        <div class="row" v-for="(row, index) in tableData.results" :key="index">
          <div class="col els">{{ divide(row.money) }}</div>
          <div class="col els">{{ row.quantity }}</div>
          <div class="col">{{ divide(row.moneyReceive) }}</div>
...
export default {
  ...
  methods: {
    ...
    async lotteryBetsOrder(obj = {}) {
      ...
      const [err, res] = await userApi.redSend(query);
      this.$toast.clear();
      this.loading = false;
      if (err) {
        this.finished = true;
