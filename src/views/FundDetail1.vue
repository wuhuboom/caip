    ...
    >
      <div class="table-lists">
        <div class="row hearder">
          <div class="col">红包发起人</div>
          <div class="col">领取金额</div>
          <div class="col">领取时间</div>
        </div>
        <div class="row" v-for="(row, index) in tableData.results" :key="index">
          <div class="col els">{{ row.fromUser }}</div>
          <div class="col els">{{ divide(row.money) }}</div>
          <div class="col">{{ $dayjsTime(row.updatedAt) }}</div>
        </div>
      </div>
    </van-list>
    <BtmActionSheet
...
export default {
  ...
  methods: {
    ...
    async lotteryBetsOrder(obj = {}) {
      ...
      const [err, res] = await userApi.redReceive(query);
      this.$toast.clear();
      this.loading = false;
      if (err) {
        this.finished = true;
